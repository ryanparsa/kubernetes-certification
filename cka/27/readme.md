...
Adjust the *Job* at `cka/10/course/backup.yaml` to use a *PVC* which request `50Mi` storage and uses the new *StorageClass*.
...
```bash
kubectl apply -f cka/10/course/backup.yaml
...
```bash
cp cka/10/course/backup.yaml cka/10/course/backup.yaml_ori

vim cka/10/course/backup.yaml
```
...
```bash
kubectl delete -f cka/10/course/backup.yaml
