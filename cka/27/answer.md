...
```bash
kubectl apply -f cka/27/lab/backup.yaml
...
```bash
cp cka/27/lab/backup.yaml cka/27/lab/backup.yaml_ori

vim cka/27/lab/backup.yaml
```
...
```bash
kubectl delete -f cka/27/lab/backup.yaml
...
- [x] Job uses PVC
- [x] PVC uses StorageClass
- [x] PVC requests required storage
- [x] Job created backups on the PVC
