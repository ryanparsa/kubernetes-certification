...
```bash
# kubectl -n project-tiger create deployment --image=httpd:2-alpine ds-important --dry-run=client -o yaml > 11.yaml
kubectl -n project-tiger create deployment --image=httpd:2-alpine ds-important --dry-run=client -o yaml > 11.yaml
```

Or we could search for a *DaemonSet* example yaml in the *Kubernetes* docs and alter it to our needs.
...
Above we can see one *Pod* on each *Node*, including the *Controlplane Node*.
