...
- Second named `container2` with image `kubernetes/pause`

There should be only **one** *Pod* of that *Deployment* running on **one** worker node. Because the *Deployment* has three replicas, one *Pod* should run on each worker node and the third won't be scheduled.
