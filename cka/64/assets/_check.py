...
if not os.environ.get("KUBECONFIG") or not os.path.exists(os.environ["KUBECONFIG"]):
    local_config = os.path.join(SCRIPT_DIR, "kubeconfig.yaml")
    if os.path.exists(local_config):
        os.environ["KUBECONFIG"] = local_config
...
class TestNetworkPolicyLab(unittest.TestCase):
    ...
    def test_connectivity(self):
        ...
        res = subprocess.run(["kubectl", "exec", "-n", "network", web_pod, "--", "wget", "-q", "-T", "2", "-O", "-", f"http://{api_ip}"], capture_output=True)
        ...
        res = subprocess.run(["kubectl", "exec", "-n", "network", web_pod, "--", "wget", "-q", "-T", "2", "-O", "-", f"http://{db_ip}:5432"], capture_output=True)
        ...
        res = subprocess.run(["kubectl", "exec", "-n", "network", api_pod, "--", "wget", "-q", "-T", "2", "-O", "-", f"http://{db_ip}:5432"], capture_output=True)
