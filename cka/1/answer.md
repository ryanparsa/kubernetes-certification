## Answer

All that's asked for here could be extracted by manually reading the kubeconfig file. But we're going to use kubectl for it.

### Step 1

First we get all context names:

```bash
kubectl --kubeconfig cka/1/lab/kubeconfig config get-contexts
CURRENT   NAME            CLUSTER     AUTHINFO               NAMESPACE
          cluster-admin   kubernetes  admin@internal         
          cluster-w100    kubernetes  account-0027@internal  
*         cluster-w200    kubernetes  account-0028@internal  

kubectl --kubeconfig cka/1/lab/kubeconfig config get-contexts -oname
cluster-admin
cluster-w100
cluster-w200

kubectl --kubeconfig cka/1/lab/kubeconfig config get-contexts -oname > cka/1/lab/contexts
```

This will result in:

```
# cka/1/lab/contexts
cluster-admin
cluster-w100
cluster-w200
```

We could also do extractions using jsonpath:

```bash
kubectl --kubeconfig cka/1/lab/kubeconfig config view -o yaml

kubectl --kubeconfig cka/1/lab/kubeconfig config view -o jsonpath="{.contexts[*].name}"
```

But it would probably be overkill for this task.

### Step 2

Now we query the current context:

```bash
kubectl --kubeconfig cka/1/lab/kubeconfig config current-context
cluster-w200

kubectl --kubeconfig cka/1/lab/kubeconfig config current-context > cka/1/lab/current-context
```

Which will result in:

```
# cka/1/lab/current-context
cluster-w200
```

### Step 3

And finally we extract the certificate and write it base64 decoded into the required location:

```bash
kubectl --kubeconfig cka/1/lab/kubeconfig config view -o yaml --raw
apiVersion: v1
clusters:
- cluster:
    certificate-authority-data: LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tC...
    server: https://10.30.110.30:6443
  name: cluster1
contexts:
- context:
    cluster: kubernetes
    user: admin@internal
  name: cluster-admin
- context:
    cluster: kubernetes
    user: account-0027@internal
  name: cluster-w100
- context:
    cluster: kubernetes
    user: account-0028@internal
  name: cluster-w200
current-context: cluster-w200
kind: Config
preferences: {}
users:
- name: account-0027@internal
  user:
    client-certificate-data: LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tCk1JSUN2RENDQWFRQ0ZIWWRqU1pGS0N5VUNSMUIybmFYQ2cvVWpTSExNQTBHQ1NxR1NJYjNEUUVCQ3dVQU1CVXgK...
    client-key-data: LS0tLS1CRUdJTiBSU0EgUFJJVkFURSBLRVk...
...
```

Instead of using `--raw` to see the sensitive certificate information, we could also simply open the kubeconfig file in an editor. No matter how, we copy the whole value of `client-certificate-data` and base64 decode it:

```bash
echo LS0tLS1CRUdJTiBDRVJUSUZJQ0FURS0tLS0tCk1JSUN2RE... | base64 -d > cka/1/lab/cert
```

Or if we like it automated:

```bash
kubectl --kubeconfig cka/1/lab/kubeconfig config view --raw -ojsonpath="{.users[?(@.name=='account-0027@internal')].user.client-certificate-data}" | base64 -d > cka/1/lab/cert
```

Which will result in:

```
# cka/1/lab/cert
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
```

Task completed.

## Killer.sh Checklist (Score: 0/4)

- [ ] `lab/contexts` contains all 3 context names (one per line)
- [ ] `lab/contexts` lists: `cluster-admin`, `cluster-w100`, `cluster-w200`
- [ ] `lab/current-context` contains: `cluster-w200`
- [ ] `lab/cert` is the base64-decoded PEM certificate for user `account-0027@internal`
