...
class TestNodePodResourceUsage(unittest.TestCase):
    ...
    def test_node_sh_exists(self):
        path = os.path.join(LAB_DIR, "node.sh")
        self.assertTrue(os.path.isfile(path), "lab/node.sh does not exist")
    ...
    def test_pod_sh_exists(self):
        path = os.path.join(LAB_DIR, "pod.sh")
        self.assertTrue(os.path.isfile(path), "lab/pod.sh does not exist")
