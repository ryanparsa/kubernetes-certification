...
```bash
kubectl run ready-if-service-ready --image=nginx:1-alpine --dry-run=client -o yaml > cka/21/lab/4_pod1.yaml
```

Next perform the necessary additions manually:

```yaml
# cka/21/lab/4_pod1.yaml
apiVersion: v1
kind: Pod
metadata:
...
```bash
kubectl create -f cka/21/lab/4_pod1.yaml
