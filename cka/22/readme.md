# Question 22 | Kustomize configure HPA Autoscaler

> **Solve this question on:** the "cka-lab-22" kind cluster
...
```
kubectl kustomize cka/22/course/api-gateway/staging | kubectl apply -f -
kubectl kustomize cka/22/course/api-gateway/prod | kubectl apply -f -
```

Using the *Kustomize* config at `cka/22/course/api-gateway` do the following:
...
*Kustomize* is a standalone tool to manage *Kubernetes* *YAML* files, but it also comes included with `kubectl`. The common idea is to have a base set of *Kubernetes* *YAML* and then override or extend it for different overlays, like here done for staging and prod:

```bash
cd cka/22/course/api-gateway
...
```yaml
# cka/22/course/api-gateway/staging/kustomization.yaml
...
```yaml
# cka/22/course/api-gateway/base/api-gateway.yaml
...
```yaml
# cka/22/course/api-gateway/prod/api-gateway.yaml
apiVersion: autoscaling/v2
kind: HorizontalPodAutoscaler
metadata:
  name: api-gateway
