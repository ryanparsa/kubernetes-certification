...
> **Solve this question on:** `ckad-lab-124`

There are existing *Namespaces* named `sun` and `moon`.

1. Create a *ServiceAccount* named `secret-manager` in *Namespace* `sun`.

2. The new *ServiceAccount* should be bound to the existing *ClusterRole* `secret-manager`. Create the appropriate *RoleBinding* named `secret-manager` in the `sun` namespace to achieve this.
